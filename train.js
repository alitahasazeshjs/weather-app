...
const fetchCities = async () => {
  ...
  cityMenuEl.innerHTML = `
    <p>Loading...</p>
  `
   const geoLoc = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${weatherInput.value}&count=10`)
  ...
  if(cities) {
    ...
    cities.forEach(city => {
      ...
      cityMenuEl.innerHTML += `
                  <div class="city-item" data-longtitude="${city.longitude}" data-latitude="${city.latitude}" data-city="${city.name}" data-admin1="${city.admin1}" data-country="${city.country}">
                      <p class="city-name">${city.name}</p>
                      <span class="city-props">${city.admin1 ? city.admin1 + ',' : ''} ${city.country}</span>
                  </div>
                `
    })
...
async function getWeather(longitude, latitude, city, admin1, country) {
  ...
  cityMenuEl.classList.add('remove');
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,relative_humidity_2m,wind_speed_10m,weather_code&hourly=temperature_2m&daily=temperature_2m_max,temperature_2m_min&wind_speed_unit=kmh`;
  ...
  if(isRes) {
    ...
    console.log("isRes if block execute!")
    noResEl.classList.add('remove')
    notFoundEl.classList.add('remove');
    container.classList.remove('remove');
  }
  const dailyTimes = res.daily.time;
  ...
  dailyEl.innerHTML = "";
  dailyWeatherArr.forEach((item, index) => {
    dailyEl.innerHTML += `
  ...
  cityDetailsEl.innerHTML = `
    <span><i class="fa-solid fa-location-dot"></i> ${admin1 === "undefined" ? '' : admin1 + ','} ${country}</span>
  `
